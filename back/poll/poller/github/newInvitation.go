...
type Invite struct {
	Invitee Invitee `json:"invite"`
}

func getInvitees(userId, owner, repo string) ([]string, error) {
	github, err := parse.GetGithubToken(userId)
	if err != nil {
		logging.ERROR.Println(err.Error())
		return nil, err
	}
	req, err := http.NewRequest("GET", fmt.Sprintf("https://api.github.com/repos/%s/%s/invitations", owner, repo), nil)
	if err != nil {
		logging.ERROR.Println(err.Error())
		return nil, err
	}
	...
	for _, item := range list {
		invitees = append(invitees, item.Invitee.Id)
	}
...
func getParamInvite(params []interface{}) *InviteParam {
	var inviteParam InviteParam

	for _, param := range params {
		myMap := param.(map[string]interface{})
		if myMap["repo-name"] != nil {
			inviteParam.Repos = myMap["repo-name"].(string)
		}
		if myMap["owner-name"] != nil {
			inviteParam.Owner = myMap["owner-name"].(string)
		}
	}
...
func newInvites(area parse.Area) {
	param := getParamInvite(area.Actions.Parameters)
	names, err := getInvitees(area.UserId, param.Owner, param.Repos)
	if err != nil {
		return
	}
	...
	_ = parse.UpdateData(area.Id, area)
	logging.INFO.Println("Data for action add invitee is update !")
